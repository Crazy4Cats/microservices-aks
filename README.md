...
- Sign in to Azure DevOps.
    ...
    <b>Having issues? </b> check the <a href="https://dev.azure.com" target="_blank">cheat link!</a>
...
- Create a new project:
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/devops/organizations/projects/create-project?view=azure-devops" target="_blank">cheat link!</a>

Step 3:
- Import the GitHub repo into the new Azure DevOps repo:

    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/devops/repos/git/import-git-repository?view=azure-devops" target="_blank">cheat link!</a>
...
- Sign in to <a href="https://portal.azure.com" target="_blank">Azure Portal </a> and open the cloud shell bash or use the <a href="https://shell.azure.com/bash" target="_blank">Azure Cloud Shell</a>.
    ...
    <b>Having issues? </b> check the <a href="https://github.com/MicrosoftDocs/azure-docs/blob/master/articles/cloud-shell/overview.md" target="_blank">cheat link!</a>
...
- Using the cloud shell bash clone the GitHub repo:
    ...
    <b>Having issues? </b> check the <a href="https://help.github.com/en/articles/cloning-a-repository" target="_blank">cheat link!</a>
...
- Execute the script configuration\container-registry\config.sh to create the Container Registry resource.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/container-registry/container-registry-get-started-portal" target="_blank">cheat link!</a>
...
- Execute the script configuration\cosmos-db\config.sh to create the CosmosDB account.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/cosmos-db/create-cosmosdb-resources-portal" target="_blank">cheat link!</a>
    <br/>
...
- Execute the script configuration\key-vault\config.sh to create the Key Vault resource.
    ...
    <b>Having issues? </b> check the <a href="resources/docs/KeyVault-for-Microservices.md" target="_blank">cheat link!</a>
...
- Execute the script configuration\kubernetes-service\config.sh to create the Kubernetes Service cluster.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/aks/kubernetes-walkthrough" target="_blank">cheat link!</a>
...
- Execute the script configuration\public-ips\config.sh to create the three public IPs required by the apps.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/aks/static-ip" target="_blank">cheat link!</a>

Step 11:
- Sign in in Azure Portal and create a new SendGrid resource.
- Manage the new SendGrid resource and Configure a new API Key.

    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/sendgrid-dotnet-how-to-send-email" target="_blank">cheat link!</a>
...
- Create a Kubernetes Service namespace with the name: microservices-aks.
    ...
    <b>Having issues? </b> check the <a href="https://www.assistanz.com/steps-to-create-custom-namespace-in-the-kubernetes/" target="_blank">cheat link!</a>
...
- Configure the docker registry secret for Kubernetes Service:
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/aks/concepts-security" target="_blank">cheat link!</a>
...
- Go to secrets\reporting-service-api and configure the settings:
    ...
    ```json
    {
        "ApplicationSettings": {
            ...
            "KeyVaultClientSecret": "KEY-VAULT-CLIENT-SECREET",
...
- In the same path folder execute:
    ...
    <b>Having issues? </b> check the <a href="https://kubernetes.io/docs/concepts/configuration/secret/" target="_blank">cheat link!</a>
...
Step 17:
- In case you want to execute locally the applications ensure you set right the appsettings for each application.
- You will need Docker installed locally to start RabbitMQ container since there are no code project for this service. 

    <b>Having issues? </b> check the <a href="https://www.freecodecamp.org/news/docker-easy-as-build-run-done-e174cc452599/" target="_blank">cheat link!</a>
...
- In Azure DevOps and set up a new Azure Resource Manager Service Connection, pointing to the subscription where is the Kubernetes Cluster resource group.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/devops/pipelines/library/service-endpoints?view=azure-devops&tabs=yaml" target="_blank">cheat link!</a>

    + Note: Ensure you have checked the option "Allow all pipelines to use this connection" when create the service connection.

Step 19:
- In Azure DevOps, create a new environment with the name: Testing Environment.

    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/devops/pipelines/process/environments?view=azure-devops" target="_blank">cheat link!</a>

Step 20:
- In Azure DevOps, select the environment with the name: Testing Environment and add approval permission.

    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/devops/pipelines/process/approvals?view=azure-devops" target="_blank">cheat link!</a>

Step 21:
- In Azure DevOps, create a new pipeline for each application: RabbitMQ, API, Processor and Website. 
    + Connect to the Azure Repo.
    + Select the right repository where are located the build-deploy yaml scripts.
    + Use the option: Existing Azure Pipelines yaml file.
    + Select the existing yaml file.
    + Run the pipeline.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/devops/pipelines/ecosystems/dotnet-core?view=azure-devops" target="_blank">cheat link!</a>

Step 22:
- In Azure DevOps, edit the pipeline and add the environment variables need it. Each script has a commented variables that need to be added in to the pipeline to run succesfully, you don't need to edit the pipeline code, this change should be directly in Azure DevOps.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/devops/pipelines/process/variables?view=azure-devops&tabs=yaml%2Cbatch" target="_blank">cheat link!</a>

Step 23:
- Repeat the steps 20 & 21 for each application that will be deployed to Kubernetes Service.

Step 24:
- Ensure the releases have been deployed successfully, pods and services must be up and running in the Kubernetes Service, validate it by opening the Kubernetes dashboard.
...
- Open the Kubernetes dashboard.
    ...
    <b>Having issues? </b> check the <a href="https://docs.microsoft.com/en-us/azure/aks/kubernetes-dashboard" target="_blank">cheat link!</a>
...
- To delete a HELM app.
    ```bash
    helm del --purge [helmPackage]
    ```

    <b>Having issues? </b> check the <a href="https://helm.sh/docs/helm/#helm" target="_blank">cheat link!</a>

Step 25:
- Congratulations, you are now able to navigate in the containerized website by using the public ip previously assigned.
